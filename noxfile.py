...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=src/saltext/zypper/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/zypper/*")
            ...
            #    "--include=src/saltext/zypper/*,tests/*"
            # )
        finally:
...
def docs(session):
    ...
    session.run("make", "linkcheck", "SPHINXOPTS=-W -v -T", external=True)
    session.run("make", "coverage", "SPHINXOPTS=-W -v -T", external=True)
